...
const getDbDeviceTypeJsons = async (): Promise<
	Record<string, DeviceTypeJson>
> => {
	...
	let dtsWithoutAssets = dtsWithAssetRelease.flatMap(({ dt, assetRelease }) =>
		assetRelease == null ? [dt] : [],
	);
...
const getProxiedDeviceTypeJsons = async (
	upstreamUrl: string,
): Promise<Record<string, DeviceTypeJson>> => {
	const response = await fetch(upstreamUrl, {
		signal: AbortSignal.timeout(EXTERNAL_HTTP_TIMEOUT_MS),
	});
	if (!response.ok) {
		throw new InternalRequestError(
			`Failed to retrieve device types from ${upstreamUrl}`,
		);
	}
	const deviceTypes = (await response.json()) as DeviceTypeJson[];
	if (!Array.isArray(deviceTypes)) {
		throw new InternalRequestError(
			`Received a non-array device types response from ${upstreamUrl}`,
		);
	}
	return Object.fromEntries(deviceTypes.map((dt) => [dt.slug, dt]));
};

export const getDeviceTypeJsons = multiCacheMemoizee(
	async (): Promise<Record<string, DeviceTypeJson>> => {
		const result =
			PROXY_DEVICE_TYPES_V1_ENDPOINTS != null
				? await getProxiedDeviceTypeJsons(PROXY_DEVICE_TYPES_V1_ENDPOINTS)
				: await getDbDeviceTypeJsons();
