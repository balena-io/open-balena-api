...
test('/user/v1/whoami returns a user', async function() {
...
test('/user/v1/whoami returns a user when using a correctly scoped access token', async function() {
...
test('/user/v1/whoami returns a 401 error when using a scoped access token that does not have user permissions', async function() {
	const record = (await supertest(app, this.token)
		.get("/v5/user?$filter=username eq 'admin'")
		.expect(200)).body.d[0];
