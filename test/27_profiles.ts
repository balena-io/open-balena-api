	...
	versions.test((version) => {
		...
		describe('profiles', function () {
			before(async function () {
				...
				this.hostApp2 = fx.applications.hostApp2;
				...
				this.hostApp2Device = fx.devices.hostApp2Device;
			...
			describe('device profile override', function () {
				it('should succeed with mandatory properties', async function () {
					...
					expect(res.body)
						.to.have.nested.property('device.__id')
						.that.equals(this.hostAppOperatedDevice.id);
					expect(res.body)
						.to.have.nested.property('on__application.__id')
						.that.equals(this.hostApp.id);
					expect(res.body.overrides__profile_name).to.equal('kernel-modules');
				...
				it('should reject an on__application the device is only operating a different release for', async function () {
					await supertest(this.user)
						.post(`/${version}/device_profile_override`)
						.send({
				...
				it('should cascade delete device profile overrides when the on__application is deleted', async function () {
					await supertest(this.user)
						.post(`/${version}/device_profile_override`)
						.send({
							device: this.hostApp2Device.id,
							overrides__profile_name: 'cascade-profile',
							on__application: this.hostApp2.id,
							is_active: true,
						})
						.expect(201);

					await supertest(this.user)
						.delete(`/${version}/application(${this.hostApp2.id})`)
						.expect(200);

					const res = await supertest(this.user)
						.get(
							`/${version}/device_profile_override?$select=id&$filter=on__application eq ${this.hostApp2.id}`,
						)
						.expect(200);
					expect(res.body).to.have.nested.property('d.length', 0);
				});
