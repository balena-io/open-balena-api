...
{
	const prefix = '/device-types/v1';
	test(`${prefix} should succeed to return a result`, async () => {
	...
	test(`${prefix} should omit releases that have an IGNORE file`, async () => {
		const res = await supertest(app)
			.get('/device-types/v1')
			.expect(200);
		expect(res.body).to.be.an('array');
	...
	test(`${prefix} should omit releases that have an IGNORE file that gives an unauthorized error`, async () => {
		const res = await supertest(app)
			.get('/device-types/v1')
			.expect(200);
		expect(res.body).to.be.an('array');
	...
	test(`${prefix} should omit releases that have an empty device-type.json`, async () => {
		const res = await supertest(app)
			.get('/device-types/v1')
			.expect(200);
		expect(res.body).to.be.an('array');
		const deviceType = _.find(
			res.body,
			deviceType => deviceType.slug == 'dt-with-empty-device-type-json-release',
		);
		expect(deviceType).to.be.an('object');
		expect(deviceType).to.have.property('buildId', '2.0.1+rev1.prod');
	});

	test(`${prefix} should omit releases that do not have a device-type.json`, async () => {
		const res = await supertest(app)
			.get('/device-types/v1')
			.expect(200);
		expect(res.body).to.be.an('array');
	...
	test(`${prefix} should succeed and omit releases that the retrieval of the IGNORE file fails`, async () => {
		const res = await supertest(app)
			.get('/device-types/v1')
			.expect(200);
		expect(res.body).to.be.an('array');
	...
	test(`${prefix} should succeed and omit releases that the retrieval of device-type.json fails`, async () => {
		const res = await supertest(app)
			.get('/device-types/v1')
			.expect(200);
		expect(res.body).to.be.an('array');
	...
	test(`${prefix} should succeed and omit device types whose details fail to be retrieved`, async () => {
		const res = await supertest(app)
			.get('/device-types/v1')
			.expect(200);
		expect(res.body).to.be.an('array');
	...
	test(`${prefix} should not contain device types with no valid releases`, async () => {
		const res = await supertest(app)
			.get('/device-types/v1')
			.expect(200);
		expect(res.body).to.be.an('array');
	...
	test(`${prefix} should not contain device types whose details fail to be retrieved`, async () => {
		const res = await supertest(app)
			.get('/device-types/v1')
			.expect(200);
		expect(res.body).to.be.an('array');
	...
	test(`${prefix} should return a proper result`, async () => {
		const res = await supertest(app)
			.get('/device-types/v1')
			.expect(200);
		expect(res.body).to.be.an('array');
...
{
	const prefix = '/device-types/v1/:deviceType';
	test(`${prefix} should return a proper result`, async () => {
		const res = await supertest(app)
	...
	test(`${prefix} should show devices types for aliases as well`, async () => {
		const res = await supertest(app)
...
{
	const prefix = '/device-types/v1/:deviceType/images';
	test(`${prefix} should return a proper result`, async () => {
		const res = await supertest(app)
	...
	test(`${prefix} should return a proper result for an alias`, async () => {
		const res = await supertest(app)
