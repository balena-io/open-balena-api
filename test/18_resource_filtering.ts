	...
	versions.test((_version, pineTest) => {
		describe('Resource Filtering', () => {
			...
			describe('Ordering by fields of reverse navigation resources', () => {
				...
				itExpectsError(
					'should order applications by the value of a specific tag that is not defined in all applications',
					async () => {
						const { body } = await pineUser.get({
							resource: 'application',
							options: {
					...
					`expected [ [ 'appapp3', 'pos:0' ], …(2) ] to deeply equal [ [ 'appapp3', 'pos:0' ], …(3) ]`,
				);
			});
		});
	});
