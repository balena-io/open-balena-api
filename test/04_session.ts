...
export default () => {
	...
	describe('scoped roles token', function () {
		...
		it('should return 401 when the bound userId does not match any user', async function () {
			const token = createScopedRolesToken({
				actor,
				roles: [ROLE_NAME],
				bindings: {},
		...
		it('should return 401 for tokens with only one of userId/jwt_secret', async function () {
			const onlySecret = createJwt(
				{ actor, roles: [ROLE_NAME], bindings: {}, jwt_secret: jwtSecret },
				{ expiresIn: EXPIRES_IN },
			);
			await supertest(onlySecret).get('/user/v1/whoami').expect(401);

			const onlyUserId = createJwt(
				{ actor, roles: [ROLE_NAME], bindings: {}, userId },
				{ expiresIn: EXPIRES_IN },
			);
			await supertest(onlyUserId).get('/user/v1/whoami').expect(401);
		});

		it('should refuse to mint a token with only one of userId/jwt_secret', function () {
			const base = {
				actor,
				roles: [ROLE_NAME],
				bindings: {},
			...
			expect(() => createScopedRolesToken({ ...base, userId } as any)).to.throw(
				'userId and jwt_secret must either both be provided',
			);
			expect(() =>
				createScopedRolesToken({ ...base, jwt_secret: jwtSecret } as any),
			).to.throw('userId and jwt_secret must either both be provided');
		});
