	...
	versions.test((version, pineTest) => {
		...
		describe('versioning releases', () => {
			...
			it('should succeed to return versioned releases', async () => {
				...
				for (const release of res.body.d as AnyObject[]) {
					expect(release)
			...
			const makeMarkAsFinalTest = (
				...
				it(`should assign unique revisions when multiple draft releases are marked as final ${titlePart}`, async () => {
					...
					for (const r of newDraftReleases) {
				...
				it(`should assign the correct revisions when changing the semver of a release ${titlePart}`, async () => {
					const [v1Releases, v2Releases] = await Promise.all(
						[SEMVER_A, SEMVER_B].map(async (semver) => {
							...
							for (const r of newReleases) {
								expectCorrectReleaseComputedTerms(r);
							}
			...
			for (const [semver, prerelease] of [
				['1.2.3-beta1', 'beta1'],
				['1.2.3-beta.2', 'beta.2'],
			...
			for (const [semver, prerelease, build] of [
		...
		describe('draft releases', () => {
			...
			for (const [semver, prerelease, variant] of [
				['1.2.3-beta1', 'beta1'],
			...
			for (const [semver, prerelease, build, resultingRevision] of [
			...
			for (const [semver, build, resultingRevision, canFinalize] of [
